...
async def hNetrWkstaGetInfo(dce, level):
    request = NetrWkstaGetInfo()
    request['ServerName'] = '\x00'*10
    request['Level'] = level
    return await dce.request(request)

async def hNetrWkstaUserEnum(dce, level, preferredMaximumLength=0xffffffff):
...
async def hNetrWkstaTransportEnum(dce, level, resumeHandle = 0, preferredMaximumLength = 0xffffffff):
...
async def hNetrWkstaSetInfo(dce, level, wkstInfo):
...
async def hNetrWorkstationStatisticsGet(dce, serviceName, level, options):
...
async def hNetrGetJoinInformation(dce, nameBuffer):
    request = NetrGetJoinInformation()
    request['ServerName'] = '\x00'*10
    request['NameBuffer'] = nameBuffer
    return await dce.request(request)

async def hNetrJoinDomain2(dce, domainNameParam, machineAccountOU, accountName, password, options):
...
async def hNetrUnjoinDomain2(dce, accountName, password, options):
...
async def hNetrRenameMachineInDomain2(dce, machineName, accountName, password, options):
...
async def hNetrValidateName2(dce, nameToValidate, accountName, password, nameType):
...
async def hNetrGetJoinableOUs2(dce, domainNameParam, accountName, password, OUCount):
...
async def hNetrAddAlternateComputerName(dce, alternateName, domainAccount, encryptedPassword):
...
async def hNetrRemoveAlternateComputerName(dce, alternateName, domainAccount, encryptedPassword):
...
async def hNetrSetPrimaryComputerName(dce, primaryName, domainAccount, encryptedPassword):
...
async def hNetrEnumerateComputerNames(dce, nameType):
    request = NetrEnumerateComputerNames()
    request['ServerName'] = '\x00'*10
    request['NameType'] = nameType
    return await dce.request(request)

async def hNetrUseAdd(dce, level, infoStruct):
    request = NetrUseAdd()
    request['ServerName'] = '\x00'*10
    request['Level'] = level
...
async def hNetrUseEnum(dce, level, resumeHandle = 0, preferredMaximumLength = 0xffffffff):
    request = NetrUseEnum()
    request['ServerName'] = '\x00'*10
...
async def hNetrUseGetInfo(dce, useName, level):
...
async def hNetrUseDel(dce, useName, forceLevel=USE_LOTS_OF_FORCE):
    request = NetrUseDel()
    request['ServerName'] = '\x00'*10
    request['UseName'] = checkNullString(useName)
