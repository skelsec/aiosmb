...
async def hDRSDomainControllerInfo(dce, hDrs, domain, infoLevel):
...
def deriveKey(baseKey):
		...
		if PY2:
			return expand_DES_key(b''.join(key1)),expand_DES_key(b''.join(key2))
		else:
...
def OidFromAttid(prefixTable, attr):
	...
	return str(ObjectIdentifier.load(b'\x06' + pack('B',(len(binaryOID))) + b''.join(binaryOID)))
