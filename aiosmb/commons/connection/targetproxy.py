...
class SMBTargetProxy:
	...
	def from_url(url_str):
		...
		if proxy.secret is not None:
			...
			proxy.secret_type = SMBTargetProxySecretType.NONE

		if proxy.ip is None:
			raise Exception('Proxy server specified with missing servber address! (add proxyhost to params)')
		
		if proxy.type in [SMBTargetProxyServerType.MULTIPLEXOR_SSL, SMBTargetProxyServerType.MULTIPLEXOR]:
	...
	def from_connection_string(s):
		...
		st.type = SMBTargetProxyServerType(o.scheme.upper().replace('+','_'))
		st.ip = o.hostname
		...
		if st.type in [SMBTargetProxyServerType.MULTIPLEXOR, SMBTargetProxyServerType.MULTIPLEXOR_SSL]:
